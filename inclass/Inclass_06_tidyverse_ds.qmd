...
```{r}
mean.age <- family_extended_city_state |>
...
```{r}
mean.age <- mean(family_extended_city_state$age)
...
family.summary <- family_complete |>
                  summarize(mean.age = mean(age,na.rm=T), 
                      mean.year = mean(year_born),
                      people = n(), people2 = sum(!is.na(age))
                      )
...
family_complete |>
  group_by(life_stage) |>
  summarize(mean.age = mean(age), min.age=min(age), max.age=max(age))
...
air_passengers <- read.csv("../..//CE594R_data_science_R/data/air_passengers.csv")
...
```{r}
air_passengers |>
  summarize(Jan.mean = mean(Jan), Feb.mean = mean(Feb))
...
```{r}
pass_summary <- air_passengers_long |>
                group_by(Month) |>
                summarize(avg = mean(Passengers), min=min(Passengers),
                          max=max(Passengers),sd=sd(Passengers)
                )
